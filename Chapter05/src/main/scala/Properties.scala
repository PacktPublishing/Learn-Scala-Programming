...
object Properties {
  ...
  object Invariants {
    ...
    forAll((a: String) => a.toSeq.sorted.unwrap.length == a.length).check
  ...
  object Idempotency {
    ...
    forAll((a: String) => a.toSeq.sorted.sorted.unwrap == a.toSeq.sorted.unwrap).check
  ...
  object TestOracle {
    forAll { a: String =>
      ...
      a.toSeq.sorted.unwrap == b
    }.check
  }

  object Checking {
    private val prop = forAll { a: String =>
      a.reverse.reverse == a
    }
    ...
    forAll { a: String =>
      classify(a.isEmpty, "empty string", "non-empty string") {
        val result = a.toSeq.sorted.unwrap.length == a.length
        result
      }
    }.check()
